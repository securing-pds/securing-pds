class TikzPlot:
    ...
    def line(self, coords, color="blue", linestyle="solid", legend_label=None, thickness=1, axes_labels=None, marker=None, **kwargs):
    ...
    def save(self, filename, xmin=None, xmax=None, ymin=None, ymax=None, axes_labels=None, figsize=None, xticks=None, yticks=None, xticklabels=None, yticklabels=None, compile=True, **kwargs):
        ...
        if isinstance(xticks, list):
            src += "\txtick = {" + ",".join(map(str,xticks)) + "},\n"
        if isinstance(yticks, list):
            src += "\tytick = {" + ",".join(map(str,yticks)) + "},\n"
        if isinstance(xticklabels, list):
            src += "\txticklabels = {" + ",".join(xticklabels) + "},\n"
        if isinstance(yticklabels, list):
            src += "\tyticklabels = {" + ",".join(yticklabels) + "},\n"
        ...
        if compile:
            ...
            os.system(f'rm {fn}.aux')
            os.system(f'rm {fn}.log')
